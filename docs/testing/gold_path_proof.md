...
- **approval-success**
  ...
  - Purpose: demonstrates that the gateway handler executes and streams a write tool once approval has been granted.
- **approval-reject**
  - Tests:
...
- **approval-timeout-late-decision**
  ...
  - Purpose: confirms timeouts persist and ignore late decisions.
- **audit-replay**
  - Tests:
...
- **smoke**
  ...
  - Purpose: ensures the orchestrator HTTP stack completes the write tool after an approval webhook, crossing at least one real HTTP boundary rather than merely renaming an in-memory unit test.
- **all**
...
- `approval-success`: `go test ./internal/gateway -run 'Test(ChatCompletionsHandlerExecutesWriteToolAfterApproval|ChatCompletionsHandlerStreamsWriteToolAfterApproval)' -count=1`
...
- `approval-timeout-late-decision`: `go test ./internal/gateway -run 'TestChatCompletionsHandlerTimeoutDuringApprovalPersistsTimeoutAndIgnoresLateDecision' -count=1`
...
- **approval-success**
  - Proves: the chat completions handler runs the approved write tool and streams results as expected.
  - Does not prove: approval delivery timing upstream of the handler or orchestrator behavior outside this handler.
- **approval-reject**
...
- **approval-timeout-late-decision**
  - Proves: timeout detection persists and ignores late webhook decisions.
...
- Any future bucket additions must documented here before the script runner includes them so the gold-path spec stays aligned with the approved backlog.
